...
export default function HowItWorksPage() {
 ...
 return <main className="min-h-screen bg-[#070D1B] text-white relative overflow-hidden font-sans"><div className="max-w-7xl mx-auto px-4 sm:px-8 md:px-10 py-8 sm:py-12 relative z-10 space-y-10 sm:space-y-12">
  ...
  <section className="bg-[#0A1428] border border-blue-500/30 rounded-2xl p-5 sm:p-8"><div className="grid md:grid-cols-3 gap-6 items-center"><div className="md:col-span-2 space-y-2"><div className="flex gap-2 text-xs font-bold text-blue-400"><Sparkles size={15}/>{t('how_engine_label')}</div><h2 className="text-xl font-bold break-words">{t('how_dynamic_title')}</h2><p className="text-sm text-slate-300">{t('how_dynamic_text')}</p></div><div className="flex flex-col gap-3"><button onClick={() => router.push(user ? '/product/new' : '/auth?next=/product/new')} className="bg-[#FF7828] text-white font-bold text-xs px-5 py-3 rounded-lg flex justify-center items-center gap-2">{t('how_test_product')}<ArrowRight size={14}/></button><button onClick={() => router.push(user ? '/product/demo-purifier-001/analysis' : '/auth?next=/product/demo-purifier-001/analysis')} className="bg-[#0C1B36] border border-slate-700 text-slate-300 font-semibold text-xs px-5 py-3 rounded-lg flex justify-center items-center gap-2"><Play size={12}/>{t('how_live_demo')}</button></div></div></section>
