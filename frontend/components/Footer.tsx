...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-slate-800 bg-[#070D1B] px-4 sm:px-8 md:px-10 py-8 w-full">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <nav className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-slate-400">
          <Link href="/how-it-works" className="hover:text-white transition-colors py-1">{t('nav_how_it_works')}</Link>
          <Link href="/sources" className="hover:text-white transition-colors py-1">{t('nav_sources')}</Link>
          <Link href="/about" className="hover:text-white transition-colors py-1">{t('nav_about_us')}</Link>
          <Link href="/assistant" className="hover:text-white transition-colors py-1">{t('nav_assistant')}</Link>
        </nav>
      </div>
      <div className="max-w-7xl mx-auto mt-6 pt-5 border-t border-slate-800/70 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-slate-500">
        ...
        <p className="text-[10px] sm:whitespace-nowrap">© {new Date().getFullYear()} NiyamVeda. {t('footer_rights')}</p>
      </div>
    </footer>
  );
}
